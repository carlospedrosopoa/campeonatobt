  ...
  try {
    ...
    if (partidasExistentes.some(partidaIniciada)) {
      return NextResponse.json(
        { error: "NÃ£o Ã© permitido montar grupos: jÃ¡ existem partidas com resultado ou em andamento nesta categoria." },
        { status: 400 }
      );
    }

    const body = await request.json().catch(() => null);
    const grupos = Array.isArray(body?.grupos) ? body.grupos : null;
    if (!grupos || grupos.length === 0) {
      return NextResponse.json({ error: "Informe a montagem manual dos grupos" }, { status: 400 });
    }
    ...
    const resultado = await dinamicaCategoriaService.montarGruposManualmente({
      ...
      categoriaId,
      grupos: grupos.map((grupo: any, index: number) => ({
