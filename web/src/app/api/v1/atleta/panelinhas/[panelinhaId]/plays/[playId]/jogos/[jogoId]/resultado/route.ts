...
export async function PATCH(
  ...
  try {
    ...
    const message = typeof error?.message === "string" ? error.message : "Não foi possível atualizar o resultado";
    const status = message === "Play não encontrado" || message === "Jogo não encontrado" ? 404 : 400;
    return NextResponse.json({ error: message }, { status });
  }
}
...
export async function DELETE(
  ...
  try {
    const { panelinhaId, playId, jogoId } = await params;
    const jogo = await panelinhasService.limparResultadoPlayJogo(panelinhaId, playId, jogoId, auth.user.id);
    return NextResponse.json(jogo, { headers: { "Cache-Control": "no-store", Vary: "Authorization" } });
  } catch (error: any) {
    const message = typeof error?.message === "string" ? error.message : "Não foi possível remover o resultado";
    const status = message === "Play não encontrado" || message === "Jogo não encontrado" ? 404 : 400;
    return NextResponse.json({ error: message }, { status });
  }
}
