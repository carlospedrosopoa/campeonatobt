...
export async function DELETE(
  ...
  try {
    const { panelinhaId, playId } = await params;
    const result = await panelinhasService.excluirPlay(panelinhaId, playId, auth.user.id);
    return NextResponse.json(result, { headers: { "Cache-Control": "no-store", Vary: "Authorization" } });
  } catch (error: any) {
    const message = typeof error?.message === "string" ? error.message : "Não foi possível excluir o play";
    const status = message === "Play não encontrado" ? 404 : 400;
    return NextResponse.json({ error: message }, { status });
  }
}
