...
export default function AdminEditarApoiadorPage() {
  ...
  useEffect(() => {
    ...
    async function carregar() {
      try {
        ...
        const lista = (await res.json()) as Apoiador[];
        const item = lista.find((a) => a.id === id);
        if (!item) throw new Error("Apoiador não encontrado");
        if (ativo) {
          setForm({
            nome: item.nome,
  ...
  async function buscarGeolocalizacao() {
    ...
    try {
      ...
      setForm(prev => ({
        ...
        endereco: data.enderecoCompleto || prev.endereco
      }));
    } catch (e: any) {
      alert(e.message);
    } finally {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSalvando(true);
    try {
      const res = await fetch(`/api/v1/torneios/${slug}/apoiadores/${id}`, {
        method: "PUT",
